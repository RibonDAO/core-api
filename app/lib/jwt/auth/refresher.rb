  ...
  module Auth
    module Refresher
      ...
      def refresh!(refresh_token:, decoded_token:, authenticatable:)
        raise Errors::MissingToken, token: 'refresh' unless refresh_token.present? || decoded_token.nil?

        existing_refresh_token = authenticatable.refresh_tokens.find_by_token(
          refresh_token
        )
        raise Errors::InvalidToken, token: 'refresh' if existing_refresh_token.blank?
