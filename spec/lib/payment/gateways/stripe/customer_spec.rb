...
RSpec.describe Payment::Gateways::Stripe::Customer do
  ...
  before do
    ...
    allow(Stripe::Customer).to receive(:create).and_return(OpenStruct.new({ id: 'cus_M3C9p6XNMXtIzN' }))
  end
    ...
    it 'calls Stripe::Customer api' do
      ...
      expect(Stripe::Customer)
        ...
        .with(email: payer.email,
              address: { city: 'Brasília', country: 'BR', state: 'DF' },
              name: payer.name, payment_method: 'pay_123',
              invoice_settings: { default_payment_method: 'pay_123' })
    end

    it 'returns a stripe_customer and a stripe_payment_method' do
      expect(setup_customer).to eq({ stripe_customer: OpenStruct.new({ id: 'cus_M3C9p6XNMXtIzN' }),
