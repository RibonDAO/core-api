...
RSpec.describe EmailLog, type: :model do
  ...
  describe '.email_already_sent?' do
    ...
    let(:sendgrid_template_name) { 'template_name' }

    context 'when email log exists for the receiver and template name' do
      before do
        create(:email_log, receiver:, sendgrid_template_name:)
      end

      it 'returns true' do
        expect(described_class.email_already_sent?(receiver:, sendgrid_template_name:)).to be true
      end
    end

    context 'when email log does not exist for the receiver and template name' do
      it 'returns false' do
        expect(described_class.email_already_sent?(receiver:, sendgrid_template_name:)).to be false
      end
    end
  end
  ...
  describe '.log' do
    ...
    it 'creates a new email log' do
      expect do
        described_class.log(sendgrid_template_name:, email_type:, receiver:)
      end.to change(described_class, :count).by(1)
    end

    it 'sets the attributes correctly' do
      email_log = described_class.log(sendgrid_template_name:, email_type:, receiver:)
      expect(email_log.sendgrid_template_name).to eq(sendgrid_template_name)
